...
0. **`npm install -g` puts claude in `~/.hermes/node/bin/` — not on PATH by default** — On systems where Hermes Node.js is installed under `~/.hermes/node/` (common in Hermes agent setups), `npm install -g` installs the `claude` binary to `~/.hermes/node/bin/claude` but the shell PATH only includes `~/.local/bin`. The install succeeds silently but `claude: command not found` results. **Fix:** symlink it:
   ```
   ln -s ~/.hermes/node/bin/claude ~/.local/bin/claude
   ```
   Then verify with `which claude && claude --version`.
