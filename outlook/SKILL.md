...
### Setting MATON_API_KEY — use `hermes config set`

**Do NOT** try to write directly to `~/.hermes/.env` — it is a protected file and writes will be denied with `Write denied`.

**Correct method** — use the `hermes config set` command:

```bash
hermes config set MATON_API_KEY your_api_key_here
```

This persists the key to `~/.hermes/.env` reliably.

### URL encoding in Python `urllib`

When using `urllib.request.Request` with OData query parameters, **always URL-encode the query string** using `urllib.parse.urlencode()`:

```python
# WRONG — raises InvalidURL: "URL can't contain control characters" (space in query)
url = 'https://gateway.maton.ai/outlook/v1.0/me/messages?$orderby=receivedDateTime desc'
...
url = f'https://gateway.maton.ai/outlook/v1.0/me/messages?{params}'
req = urllib.request.Request(url)
```

### Connection ID — always pass `Maton-Connection` header

List connections first to get the active `connection_id`, then use it in all API calls:

```python
import urllib.request, json

with open(os.path.expanduser("~/.hermes/.env")) as f:
    key = next(line.strip().split("=", 1)[1] for line in f if line.startswith("MATON_API_KEY"))

req = urllib.request.Request('https://ctrl.maton.ai/connections?app=outlook&status=ACTIVE')
req.add_header('Authorization', f'Bearer {key}')
connections = json.load(urllib.request.urlopen(req))['connections']
conn_id = connections[0]['connection_id']

req = urllib.request.Request(f'https://gateway.maton.ai/outlook/v1.0/me/messages?{params}')
