---
name: weread-stats
description: 微信读书统计 — 读取 Obsidian Weread 笔记库，生成阅读统计数据。触发词：「微信读书统计」「weread stats」「读书统计」「我的藏书统计」
---

# 微信读书统计

从 Obsidian 本地 Weread 笔记库读取统计数据，输出藏书规模、阅读时长分布、活跃度等信息，**自动生成可视化图表**（词云、领域柱图、双词组合、月度热力图）。

## Obsidian Weread 路径

```
~/Library/Mobile Documents/iCloud~md~obsidian/Documents/oneinall/raw/Weread/
```
...
t=len(files); s=sum(os.path.getsize(f) for f in files)
l=sum(open(f,errors='ignore').read().count('\n') for f in files)
print(f"藏书：{t} 本 | 笔记：{s/1024/1024:.1f} MB | 行数：{l:,}")
EOF
```

## 完整分析脚本

> **前置依赖**：`pip3 install jieba wordcloud matplotlib numpy pillow -q`
> 
> **字体**：词云使用 `/System/Library/Fonts/STHeiti Medium.ttc`（macOS 内置黑体），matplotlib 使用 `STHeiti Medium` + `Hiragino Sans GB`。

### 脚本：weread-full-stats.py
...
total_lines = sum(open(f, errors='ignore').read().count('\n') for f in files)
        ...
        if len(parts) >= 3:
            try:
                ...
                if progress == '100%' and finished:
                    book_finished.append({'date': finished, 'title': title,
                                         'author': str(meta.get('author', '')),
                                         'readingTime': str(meta.get('readingTime', '')),
                                         'noteCount': int(meta.get('noteCount', 0) or 0)})
            except:
...
# 6e. 阅读时长分布柱图
print("📊 生成阅读时长分布图...")
bins = [0,30,60,120,300,600,9999]
labels_bin = ['<30分钟','30-60分','1-2h','2-5h','5-10h','>10h']
counts_bin = [0]*len(bins)
for b in book_finished:
    rt = b.get('readingTime','')
    mins = 0
    if '小时' in rt:
...
| 坑 | 解法 |
|----|------|
| `ModuleNotFoundError: No module named 'jieba'` | 用 `python3` 而非 sandbox：先 `pip3 install jieba wordcloud matplotlib numpy pillow -q`，然后 `python3 weread-full-stats.py` |
| 词云字体找不到 | macOS 用 `/System/Library/Fonts/STHeiti Medium.ttc`，Linux 换成 `simhei` 或系统中文 TTF |
| `finishedDate` 是 `datetime.date` 对象 | 调用前先判 `hasattr(fd, 'strftime')` 再 `.strftime('%Y-%m-%d')` |
| CDN 图片链接污染双词 | stopwords 加 `fisheryounggod_picsbed`、`picsbed_uPic`、`https_cdn` 等 |

## 交付流程

1. 跑脚本 → 输出 `~/hermes/cards/` 下 5 张图
2. 用 `ljg-card -i` 把综合统计做成信息图，或直接发送这 5 张图给用户
