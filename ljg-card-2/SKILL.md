...
对于统计类内容（如词云），用 Python matplotlib + wordcloud 比 HTML 捕获更稳定：
...
# macOS 中文黑体路径（STHeiti Medium.ttc 比通用路径更可靠）
...
wc.to_file("~/hermes/cards/wordcloud.png")
```

> ⚠️ 如 `WordCloud(font_path=...)` 报错 `cannot open resource`，说明字体路径不对。macOS 用 `STHeiti Medium.ttc`，Linux 用 `simhei` 或 `NotoSansCJK`。
