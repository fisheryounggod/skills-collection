...
name: opencli
description: |
  Use opencli CLI to interact with social/content websites (Bilibili, Zhihu, Twitter/X, YouTube, Weibo, 小红书, V2EX, Reddit, HackerNews, 雪球, BOSS直聘 etc.) via the user's Chrome login session. ALWAYS prefer opencli over playwright/browser automation for these supported sites. FALLBACK for search: use ddg-search MCP when opencli search fails. Triggers: user asks to browse, search, fetch hot/trending content, post, or read messages on any supported site; 查B站热门, 搜知乎, 看微博热搜, 发推, 搜YouTube, 查股票行情 etc.
---
...
opencli 有对应命令？
  ...
  │         ↓
  │      执行失败/报错？
  │         ├─ 搜索类任务 → 用 ddg-search MCP
  │         └─ 其他任务 → 用 Playwright MCP
...
### 搜索任务 Fallback: ddg-search

当 `opencli <site> search` 失败时，使用 **ddg-search MCP**：

```bash
# DuckDuckGo 搜索格式（site: 限定特定网站）
ddg-search "<query> site:youtube.com"
ddg-search "<query> site:bilibili.com"
ddg-search "<query> site:zhihu.com"
ddg-search "<query> site:twitter.com OR site:x.com"
ddg-search "<query> site:reddit.com"
ddg-search "<query> site:xiaohongshu.com"  # 小红书
```

**示例映射**：
| opencli 失败命令 | ddg-search 替代 |
|----------------|-----------------|
| `opencli youtube search --query "AI tutorial"` | `ddg-search "AI tutorial site:youtube.com"` |
| `opencli bilibili search --keyword "大模型"` | `ddg-search "大模型 site:bilibili.com"` |
| `opencli zhihu search --keyword "Rust"` | `ddg-search "Rust site:zhihu.com"` |
| `opencli twitter search --query "Claude"` | `ddg-search "Claude site:twitter.com OR site:x.com"` |
