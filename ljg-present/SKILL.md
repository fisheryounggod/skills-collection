...
当内容是词汇表、术语集（如200词记忆、品类词汇等），每页可承载4–8个词条。允许多行布局，字号降档（`clamp(28px, 5vmin, 72px)`），每页顶部可加模块标签（如「01 材质」）。这是唯一允许"多行"的场景——词汇页本质是速查表，不是思想节拍。

## 交付给 Fisher 的流程

**重要**：生成 HTML 到 `~/Downloads/` 后，不要只报路径。Fisher 的偏好是"直接发内容"，步骤：

1. 写 HTML 文件到 `~/Downloads/{name}-高桥流.html`
2. **同时**调用 `send_message` 把交付信息发到 Fisher 的 WeChat（`o9cq802nxSy8F1d7Vc65-XgAe83E@im.wechat`），包含：文件路径、页数、核心节拍概览
3. 如果是 URL 内容（如 X 帖子），先用 `baoyu-url-to-markdown` 抓取，再进入提炼流程
