...
## Fisher 交付流程（X URL → 高桥流）

X URL → baoyu-url-to-markdown → markdown 内容 → ljg-present 提炼 → HTML → send_message 到 WeChat

### 典型调用

```bash
cd /Users/mac/.hermes/skills/openclaw-imports/baoyu-url-to-markdown
bun scripts/vendor/baoyu-fetch/src/cli.ts "<x-url>" \
  --output /tmp/x-post.md --timeout 45000
```

### 超时说明

Chrome 首次启动约需 30–60s，建议 `--timeout 45000` 起。第二次调用复用已启动的 Chrome，速度快很多。
...
**Current guidance (2025-05):** Do NOT assume X Articles fail. The baoyu Chrome profile at `~/Library/Application Support/baoyu-skills/chrome-profile` is often already logged into X. Using `--wait-for interaction` successfully renders X Articles — the existing Chrome session carries the valid X session cookie.

**Recommended approach for X Article URLs:**
```bash
cd /Users/mac/.hermes/skills/openclaw-imports/baoyu-url-to-markdown
bun scripts/vendor/baoyu-fetch/src/cli.ts \
  "https://x.com/i/article/{ARTICLE_ID}" \
  --output /tmp/x-article.md \
  --wait-for interaction \
  --timeout 120000 \
  --format json
```

**When using `--format json`:** The markdown content is nested inside the `"markdown"` field. Extract it before passing to downstream tools:
```python
import json
with open('/tmp/x-article.json') as f:
    d = json.load(f)
with open('/tmp/x-article-clean.md', 'w') as f:
    f.write(d['markdown'])
```

**One-liner pipeline (X Article → clean markdown):**
```bash
bun scripts/vendor/baoyu-fetch/src/cli.ts "https://x.com/i/article/{ID}" \
  --output /tmp/x-article.json --wait-for interaction --timeout 120000 --format json \
  && python3 -c "import json,sys; d=json.load(open('/tmp/x-article.json')); open('/tmp/x-article-clean.md','w').write(d['markdown'])"
```

**What to expect:** Chrome reuses the existing session at `http://127.0.0.1:60267`. The article renders fully after a brief wait. Exit code 0 with `"status": "ok"` in JSON means success. If the session is not logged in, the output will reflect that — try signing into X in the Chrome window that opens.

**Note:** The older `vxtwitter` API approach only returns metadata (title, author, preview text) for tweet URLs, not article body content. It cannot replace Chrome rendering for X Articles.

**Fallback: vxtwitter API** — When the X adapter fails, try `https://api.vxtwitter.com/{handle}/status/{id}` to get structured metadata (title, author, preview text, media URLs). Works without login for public posts. Example:
```bash
curl -sL "https://api.vxtwitter.com/pandatalk8/status/2053791188009042389"
```

**Fisher's Chrome CDP** — Chrome runs at `http://127.0.0.1:60267` with profile `~/Library/Application Support/baoyu-skills/chrome-profile`. baoyu-fetch auto-reuses this instance. To manually create a new tab:
```bash
curl -X PUT "http://127.0.0.1:60267/json/new" -H "Content-Type: application/json" -d '{"url":"about:blank"}'
```
Do NOT use `--cdp-url` to override — baoyu-fetch manages its own CDP lifecycle.
