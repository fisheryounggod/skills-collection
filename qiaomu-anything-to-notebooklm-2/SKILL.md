...
如果 `notebooklm login` 报 `TargetClosedError` 或浏览器闪退：
```bash
...
### 4. source add 的 --notebook 参数

`--notebook`（或 `-n`）接受的是 **notebook ID**，不是笔记本标题。如果只知道标题，先创建或列出 notebooks 获取 ID：

```bash
notebooklm create "我的笔记本"          # 创建新笔记本，返回 ID
notebooklm list                          # 列出所有笔记本，每行第一个字段是 ID
notebooklm source add "/path/to/file.md" -n <ID>   # 用 ID 添加源文件
```
