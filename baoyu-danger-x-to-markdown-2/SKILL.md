...
### Alternative: `summarize` CLI (fallback when baoyu script fails)

If `bun main.ts` fails with a `SyntaxError: Export named 'gracefulKillChrome'` (broken `baoyu-chrome-cdp` import), use the `summarize` CLI as a fallback:

```bash
summarize "https://x.com/USERNAME/status/TWEET_ID" 2>&1
```

`summarize` works on:
- X tweet URLs (article-format tweets resolve to `https://x.com/i/article/<id>` internally)
- Full X article URLs: `https://x.com/i/article/<id>`

Extract the article slug from t.co shortlinks first:
```bash
curl -sL --max-time 10 -L "https://t.co/SHORTLINK" -w "%{url_effective}" -o /dev/null
```

> Note: `summarize` outputs structured text + metadata but no YAML frontmatter. Manually add frontmatter after extraction.

### Troubleshooting baoyu-x-to-markdown

**`SyntaxError: Export named 'gracefulKillChrome'`** — broken `baoyu-chrome-cdp` dependency:
```bash
cd ~/.hermes/skills/openclaw-imports/baoyu-danger-x-to-markdown/scripts && npm install baoyu-chrome-cdp
```
If still broken after reinstall, fall back to `summarize` CLI above.

**xurl 401 Unauthorized** — X API credentials are NOT set up (this is distinct from `xurl` being installed). Fall back to `summarize` CLI for reading public posts.
